...
export class userInterface {
  ...
  renderMainContent() {
    ...
    switch (this.currentFilter) {
      case "today":
        ...
        break;
      case "week":
        todos = this.currentProject.todos.filter((todo) =>
          todo.isDueThisWeek(),
        );
        break;
      default:
  ...
  handleDropOnTodo(e, targetTodoId) {
    ...
    const sourceProject = this.projects.find(
      (p) => p.id === this.draggedTodo.sourceProjectId,
    );
    ...
    if (sourceProject && targetTodo) {
      ...
      const afterElement = this.dragDropHelper.getDragAfterElement(
        todosContainer,
        e.clientY,
      );

      const draggedTodoIndex = sourceProject.todos.findIndex(
        (t) => t.id === this.draggedTodo.id,
      );
      const targetTodoIndex = this.currentProject.todos.findIndex(
        (t) => t.id === targetTodoId,
      );
      ...
      if (draggedTodoIndex !== -1 && targetTodoIndex !== -1) {
        ...
        if (afterElement) {
          ...
          const afterIndex = this.currentProject.todos.findIndex(
            (t) => t.id === afterId,
          );
  ...
  showProjectModal(projectId = null) {
    ...
    document.getElementById("project-modal-title").textContent = this
  ...
  showTodoModal(todoId = null) {
    this.currentEditingTodo = todoId
      ? this.currentProject.getTodo(todoId)
      : null;

    document.getElementById("modal-title").textContent = this.currentEditingTodo
      ? "Edit Task"
      : "New Task";
